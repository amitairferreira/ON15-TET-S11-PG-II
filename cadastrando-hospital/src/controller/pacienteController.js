...
const todosPacientes = (request, response) => {
  response.status(200).json({
    mensagem: 'Essses são os pacientes cadastrados',
    pacientes
  })
}
...
const buscarPorId = (request, response) => {
  try {
    ...
    //entrando no bd, achando o id no banco e comparando com o id digitado
...
//listar por nome, se tiver social, trazer nome social (GET)
  ...
  try {
    ...
    const encontrarNome = pacientes.filter(paciente => {
      if (paciente.nomeSocial) {
        return paciente.nomeSocial.toLocaleLowerCase().includes(trazerNome)
      }
      return paciente.nome.toLocaleLowerCase().includes(trazerNome)
    })
